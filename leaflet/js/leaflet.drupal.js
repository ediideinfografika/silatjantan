(function ($, Drupal, drupalSettings) {
  ...
  Drupal.Leaflet.prototype.initialise = function (mapid) {
    ...
    for (let key in self.map_definition.layers) {
      let layerx = self.map_definition.layers[key];
      // Distinguish between "base" and "overlay" layers.
      // Default to "base" in case "layer_type" has not been defined in hook_leaflet_map_info().
      layerx.layer_type = (typeof layerx.layer_type === 'undefined') ? 'base' : layerx.layer_type;

      switch (layerx.layer_type) {
        case 'overlay':
          let overlay_layer = self.create_layer(layerx, key);
          let layer_hidden = (typeof layerx.layer_hidden === "undefined") ? false : layerx.layer_hidden;
          self.add_overlay(key, overlay_layer, layer_hidden, mapid);
          break;

        default:
          self.add_base_layer(key, layerx, i, mapid);
          // Only the first base layerx needs to be added to the map - all the
          // others are accessed via the layerx switcher.
          if (i === 0) {
            i++;
          }
          break;
    ...
    if (self.settings.fullscreen && self.settings.fullscreen.control) {
      ...
      L.control.measure().addTo(self.lMap);

      if (window.location.pathname != '/node/43464') {
        var legendx = '<i style="background: yellow"></i><span>Lokal Primer</span><br> <i style="background: #c55a11"></i><span>Kolektor Primer - 4</span>';
        var markersLayer = new L.LayerGroup();
        fetch("https://silatjantan.dpupr.madiunkab.go.id/sites/default/files/karakteristik_jalan.geojson")
          ...
          .then(function (data) {
            ...
            self.lMap.addLayer(featuresLayer);
            featuresLayer.bringToBack();
            ...
            searchControl.on('search:locationfound', function (e) {
              ...
              // jQuery('#edit-field-nama-0-value').val(e.layer.feature.properties.Nm_Ruas);
              // jQuery('#edit-field-status-0-value').val(e.layer.feature.properties.Status);
              // jQuery('#edit-field-fungsi-0-value').val(e.layer.feature.properties.Fungsi);
            ...
            self.lMap.addControl(searchControl);  //inizialize search control
  ...
  Drupal.Leaflet.prototype.initialise_layer_control = function (mapid) {
    ...
    // Only add a layerx switcher if it is enabled in settings, and we have
    ...
    if (Drupal.Leaflet[mapid].layer_control == null && self.settings.layerControl && (count_layers(self.base_layers) > 1 || count_layers(self.overlays) > 0)) {
      // Instantiate layerx control, using settings.layerControl as settings.
  ...
  Drupal.Leaflet.prototype.add_base_layer = function (key, definition, i, mapid) {
    ...
    // Only the first base layerx needs to be added to the map - all the others are accessed via the layerx switcher.
    if (i === 0) {
    ...
    if (Drupal.Leaflet[mapid].layer_control == null) {
      ...
      // If we already have a layerx control, add the new base layerx to it.
      Drupal.Leaflet[mapid].layer_control.addBaseLayer(map_layer, key);
    }
    Drupal.Leaflet[mapid].base_layers = self.base_layers;
  };

  Drupal.Leaflet.prototype.add_overlay = function (label, layerx, layer_hidden, mapid) {
    let self = this;
    self.overlays[label] = layerx;
    if (!layer_hidden) {
      Drupal.Leaflet[mapid].lMap.addLayer(layerx);
    }
    ...
    if (Drupal.Leaflet[mapid].layer_control == null) {
      ...
      // If we already have a layerx control, add the new overlay to it.
      Drupal.Leaflet[mapid].layer_control.addOverlay(layerx, label);
    }
  ...
  Drupal.Leaflet.prototype.add_features = function (mapid, features, initial) {
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      if (feature.group) {
        ...
        // Add the group to the layerx switcher.
  ...
  Drupal.Leaflet.prototype.create_layer = function (layerx, key) {
    let self = this;
    let map_layer = new L.TileLayer(layerx.urlTemplate);
    if (layerx.type === 'wms') {
      map_layer = new L.tileLayer.wms(layerx.urlTemplate, layerx.options);
    }
    map_layer._leaflet_id = key;

    if (layerx.options) {
      for (let option in layerx.options) {
        map_layer.options[option] = layerx.options[option];
      }
    }

    // Layers served from TileStream need this correction in the y coordinates.
    // TODO: Need to explore this more and find a more elegant solution.
    if (layerx.type === 'tilestream') {
  ...
  Drupal.Leaflet.prototype.create_json = function (json, events) {
    ...
    lJSON.options.onEachFeature = function (feature, layerx) {
      for (let layer_id in layerx._layers) {
        for (let i in layerx._layers[layer_id]._latlngs) {
          Drupal.Leaflet.bounds.push(layerx._layers[layer_id]._latlngs[i]);
        }
      }
      if (feature.properties.style) {
        layerx.setStyle(feature.properties.style);
      }
      if (feature.properties.leaflet_id) {
        layerx._leaflet_id = feature.properties.leaflet_id;
      }
      if (feature.properties.popup) {
        layerx.bindPopup(feature.properties.popup);
      }
      for (e in events) {
        layerParam = {};
        layerParam[e] = eval(events[e]);
        layerx.on(layerParam);
      }
    };
