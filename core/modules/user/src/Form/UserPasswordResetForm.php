...
class UserPasswordResetForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, AccountInterface $user = NULL, $expiration_date = NULL, $timestamp = NULL, $hash = NULL) {
    if ($expiration_date) {
      $form['message'] = ['#markup' => $this->t('<p>Ini Adalah one-time login untuk user  %user_name and will expire on %expiration_date.</p><p>Klik tombol ini untuk masuk ke situs dan mengubah kata sandi Anda.</p>', ['%user_name' => $user->getAccountName(), '%expiration_date' => $expiration_date])];
      $form['#title'] = $this->t('Reset password');
    }
    else {
      // No expiration for first time login.
      $form['message'] = ['#markup' => $this->t('<p>Ini Adalah one-time login for %user_name.</p><p>Klik tombol ini untuk masuk ke situs dan mengubah kata sandi Anda.</p>', ['%user_name' => $user->getAccountName()])];
      $form['#title'] = $this->t('Set password');
    }

    $form['help'] = ['#markup' => '<p>' . $this->t('Login ini hanya dapat digunakan sekali') . '</p>'];
